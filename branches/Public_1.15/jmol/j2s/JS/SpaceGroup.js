...
xyz = xyz.toLowerCase ();
return (xyz.indexOf ("[[") < 0 && xyz.indexOf ("x4") < 0 && (xyz.indexOf ("x") < 0 || xyz.indexOf ("y") < 0 || xyz.indexOf ("z") < 0) ? -1 : xyz.startsWith ("c=") ? this.addCentering (xyz.substring (2)) : this.addOperation (xyz, opId, allowScaling));
}, "~S,~N,~B");
...
var s = this.getCanonicalSeitzList ();
return JS.SpaceGroup.findSpaceGroup (s);
});
Clazz.defineMethod (c$, "getCanonicalSeitzList", 
 function () {
var list =  new Array (this.operationCount);
for (var i = 0; i < this.operationCount; i++) list[i] = JS.SymmetryOperation.dumpCanonicalSeitz (this.operations[i]);
...
if (notionalUnitcell == null) return JS.SpaceGroup.determineSpaceGroup (name, 0, 0, 0, 0, 0, 0, -1);
return JS.SpaceGroup.determineSpaceGroup (name, notionalUnitcell[0], notionalUnitcell[1], notionalUnitcell[2], notionalUnitcell[3], notionalUnitcell[4], notionalUnitcell[5], -1);
}, "~S,~A");
...
if (magRev != -2) newOp.timeReversal = op.timeReversal * magRev;
